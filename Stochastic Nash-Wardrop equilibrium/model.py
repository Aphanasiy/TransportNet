...
def torch_model_solve(graph, graph_correspondences, total_od_flow,
                solver_name = 'ustf',
                gamma = 1.0, mu = 0.25, rho = 1.0, 
                epsilon = 1e-3, max_iter = 1000, verbose = False):
    if use_torch and solver_name == 'ustf':
        solver_func = ustf.universal_similar_triangles_function_with_torch
        phi_small_solver_func = pss.TorchPhiSmallSolver
        starting_msg = 'Universal similar triangles function with torch...'
    elif solver_name == 'lbfgs':
        solver_func = None #...
        phi_small_solver_func = None #...
        starting_msg = None #''
    else:
        print('Define function!')
